...
from model_sasrec_movie import SASREC
from model_fdsa_movie import FDSA
from model_nova_movie import NOVA
from model_dif_movie import DIF
from model_maif_movie import MAIF
import numpy as np
import time
...
def generate_train_batch(batch_data_index, sku_info_map, user_info_map):
    ...
    year = np.reshape([sku_info_map[item_list[i]]['year'] for i in batch_data_index], [-1, 1])
    ...
    for l in his_item_seq:
        ...
        his_year_seq.append([sku_info_map[i]['year'] for i in l])
    ...
    gender = np.reshape([user_info_map[user_list[i]]['gender'] for i in batch_data_index], [-1, 1])
    age = np.reshape([user_info_map[user_list[i]]['age'] for i in batch_data_index], [-1, 1])
    occupation = np.reshape([user_info_map[user_list[i]]['occupation'] for i in batch_data_index], [-1, 1])

    return [item, cate1, cate2, year, stars, his_item_seq, his_cate1_seq, his_cate2_seq, his_year_seq, his_stars_seq, label, user, gender, age, occupation]
...
def generate_valid_batch(batch_size, global_index, sku_info_map, user_info_map):
    ...
    year = np.reshape([sku_info_map[item_valid_list[i]]['year'] for i in range(global_index-batch_size, global_index)], [-1, 1])
    ...
    gender = np.reshape([user_info_map[user_valid_list[i]]['gender'] for i in range(global_index - batch_size, global_index)], [-1, 1])
    age = np.reshape(
        [user_info_map[user_valid_list[i]]['age'] for i in range(global_index - batch_size, global_index)],
        [-1, 1])
    occupation = np.reshape(
        [user_info_map[user_valid_list[i]]['occupation'] for i in range(global_index - batch_size, global_index)],
        [-1, 1])
...
def generate_test_batch(batch_size, global_index, sku_info_map, user_info_map):
    ...
    year = np.reshape(
        [sku_info_map[item_test_list[i]]['year'] for i in range(global_index - batch_size, global_index)], [-1, 1])
    stars = np.reshape(
    ...
    gender = np.reshape(
        [user_info_map[user_test_list[i]]['gender'] for i in range(global_index - batch_size, global_index)], [-1, 1])
    age = np.reshape(
        [user_info_map[user_test_list[i]]['age'] for i in range(global_index - batch_size, global_index)],
        [-1, 1])
    occupation = np.reshape(
        [user_info_map[user_test_list[i]]['occupation'] for i in range(global_index - batch_size, global_index)],
        [-1, 1])

    return [item, cate1, cate2, year, stars, his_item_seq, his_cate1_seq, his_cate2_seq, his_year_seq, his_stars_seq,
            label, user, gender, age, occupation]
        ...
        for i in range(input_format['num_epoch']):
            ...
            if i % 1 == 0:
                ...
                while global_index <= len(user_valid_list):
                    ...
                    uid_l = np.reshape(batch_data[11], [-1]).tolist()
                    label_l = np.reshape(batch_data[10], [-1]).tolist()
...
if len(sys.argv) > 1:
    ...
    data_type = 'movie'
    file_path_train = 'movie_traindata_negall_side_200.txt'
    file_path_valid = 'movie_validdata_08_side_200.txt'
    file_path_test = 'movie_testdata_08_side_200.txt'
    sku_info_file = 'movie_sku_info.npz'
    user_info_file = 'movie_user_info.npz'
    mode = 'train'
    model_name = 'FDSA'
    exp_name = 'fdsa_movie_batch256_dim16'
...
if mode == 'train':
    ...
    best_model_path = 'sasrec_movie_batch256_dim16/best_model/'
    read_test_file(file_path_test)
