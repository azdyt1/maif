...
class DIF(object):
    ...
    def __init__(self, input_format):
        ...
        self.seq_his_join_side_attn = self.get_sequence_embedding_with_transformer_feature('his_feature_short_seq', self.hisids, T=self.params['his_short_seq_max_len'], target_embedding=self.target_join_in, mode='mean', seq_embedding=[self.seq_his_item_in, self.seq_his_cate1_in, self.seq_his_cate2_in, self.seq_his_review_in, self.seq_his_city_in, self.seq_his_stars_in], target_seq_embedding=self.seq_his_item_in)

        # predicting layer
        self.deep_out = tf.concat([self.user_emb_join, self.target_join_in, self.seq_his_join_side_attn], 1)
        ...
        self.loss_ctr = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
           labels=self.label, logits=self.final_layer_out))

        self.label_cate = tf.tile(self.cate1id, [1, 2000])

        self.range_matrix = tf.tile(tf.expand_dims(tf.range(0, 2000, dtype=tf.int32), 0),
                                    [tf.shape(self.label_cate)[0], 1])
        self.label_cate = tf.cast(tf.where(tf.equal(self.range_matrix, self.label_cate), tf.ones_like(self.label_cate),
                                           tf.zeros_like(self.label_cate)), dtype=tf.float32)
        self.prediction_cate = tf.layers.dense(self.seq_his_join_side_attn, units=2000, activation=tf.nn.relu)
        self.loss_cate = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(logits=self.prediction_cate, labels=self.label_cate))

        self.loss = self.loss_ctr + 0.1 * self.loss_cate
    ...
    def self_multi_head_attn(self, queries,
        ...
        return outputs, weights_origin

    def self_multi_head_attn_dif(self, queries,
                             keys,
                             values,
                             num_units=None,
                             num_heads=1,
        ...
        # # Dropouts
        # # if is_drop:
        new_weights = tf.layers.dropout(new_weights, rate=self.drop_rate)

        new_weights = tf.nn.softmax(new_weights)

        # Weighted sum
        outputs = tf.matmul(new_weights, V_)
    ...
    def get_sequence_embedding_with_transformer_feature(self, scope_name, seq_ids, T=50, target_embedding=None,mode='mean',k=25,flag='attention', seq_embedding=None, target_seq_embedding=None):
        ...
        for i in range(len(seq_embedding)):
            ...
            output, weights = self.self_multi_head_attn(queries=seq_embedding[i],
                                            keys=seq_embedding[i],
                                            values=seq_embedding[i],
                                            num_units=self.params['block_shape'][0],
                                            num_heads=self.params['heads'],
                                            is_drop=self.params['is_drop'],
                                            dropout_keep_prob=self.params['dropout_keep_prob']
        ...
        # self-attention encoding
        seq_emb = self.self_multi_head_attn_dif(queries=target_seq_embedding,
                                            keys=target_seq_embedding,
                                            values=target_seq_embedding,
                                            num_units=self.params['block_shape'][0],
                                            num_heads=self.params['heads'],
                                            is_drop=self.params['is_drop'],
                                            dropout_keep_prob=self.params['dropout_keep_prob'],
                                            new_weights=attn_mat,
                                            )
