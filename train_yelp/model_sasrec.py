...
class SASREC(object):
    ...
    def __init__(self, input_format):
        ...
        self.user_emb_join = tf.concat([self.user_emb, self.user_review_emb, self.user_stars_emb, self.user_year_emb], -1)
        ...
        self.seq_his_join_in = tf.concat([self.seq_his_item_in, self.seq_his_cate1_in, self.seq_his_cate2_in, self.seq_his_review_in, self.seq_his_city_in, self.seq_his_stars_in], -1)
    ...
    def get_sequence_embedding_with_transformer(self, scope_name, seq_ids, T=50, target_embedding=None,mode='mean',k=25,flag='attention', seq_embedding=None, user_embedding=None):
        ...
        seq_emb = self.self_multi_head_attn(queries=seq_emb,
                                            keys=seq_emb,
                                            values=seq_emb,
                                            num_units=self.params['block_shape'][0]*6,
        ...
        # target attention pooling
            ...
            flat = self.attention_sum_pooling(target_embedding, seq_emb, dims=self.params['block_shape'][-1]*6, flag=flag)
        else:
            flat = tf.reduce_mean(seq_emb, axis=1)
        return flat
