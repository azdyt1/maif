...
class NOVA(object):
    ...
    def __init__(self, input_format):
        ...
        # self.seq_his_join_side_attn = self.get_sequence_embedding_with_transformer('his_short_seq', self.hisids, T=self.params['his_short_seq_max_len'], target_embedding=self.target_join_in, mode='mean', seq_embedding=self.seq_his_item_in, user_embedding=None)
    ...
    def get_sequence_embedding_with_transformer_feature(self, scope_name, seq_ids, T=50, target_embedding=None,mode='mean',k=25,flag='attention', seq_embedding=None, target_seq_embedding=None):
        ...
        seq_emb = tf.concat(seq_embedding, -1)
        seq_emb = tf.layers.dense(seq_emb, self.params['dims'], activation=tf.nn.relu)
        # self-attention encoding
        seq_emb = self.self_multi_head_attn(queries=seq_emb,
                                            keys=seq_emb,
        ...
        # target attention pooling
            ...
            flat = self.attention_sum_pooling(tf.layers.dense(target_embedding, self.params['dims']), seq_emb, dims=self.params['block_shape'][-1], flag=flag)
        else:
            flat = tf.reduce_mean(seq_emb, axis=1)
        return flat
