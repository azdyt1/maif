...
class SASREC(object):
    ...
    def __init__(self, input_format):
        ...
        self.emb_age = tf.get_variable('age_embedding', shape=[self.params['age_num'], self.params['dims']],
                                          ...
                                          initializer=tf.contrib.layers.variance_scaling_initializer(factor=2.0,
                                                                                                     mode='FAN_OUT',
                                                                                                     uniform=True))
        self.emb_occupation = tf.get_variable('occupation_embedding', shape=[self.params['occupation_num'], self.params['dims']],
                                          trainable=True,
                                          initializer=tf.contrib.layers.variance_scaling_initializer(factor=2.0,
                                                                                                     mode='FAN_OUT',
                                                                                                     uniform=True))
        ...
        self.user_gender_emb = tf.reshape(tf.nn.embedding_lookup(self.emb_gender, self.gender_id), shape=[-1, self.params['dims']])
        self.user_age_emb = tf.reshape(tf.nn.embedding_lookup(self.emb_age, self.age_id), shape=[-1, self.params['dims']])
        self.user_occupation_emb = tf.reshape(tf.nn.embedding_lookup(self.emb_occupation, self.occupation_id),shape=[-1, self.params['dims']])
        ...
        self.target_stars_in = tf.reshape(tf.nn.embedding_lookup(self.emb_stars, self.starsid),
                                          shape=[-1, self.params['dims']])
        self.target_join_in = tf.concat([self.target_item_in, self.target_cate1_in, self.target_cate2_in, self.target_year_in, self.target_stars_in], -1)
        ...
        self.seq_his_join_in = tf.concat([self.seq_his_item_in, self.seq_his_cate1_in, self.seq_his_cate2_in, self.seq_his_year_in, self.seq_his_stars_in], -1)
        ...
        # predicting layer
        self.deep_out = tf.concat([self.user_emb_join, self.target_join_in, self.seq_his_join_attn], 1)
    ...
    def get_sequence_embedding_with_transformer(self, scope_name, seq_ids, T=50, target_embedding=None,mode='mean',k=25,flag='attention', seq_embedding=None, user_embedding=None):
        ...
        seq_emb = self.self_multi_head_attn(queries=seq_emb,
                                            keys=seq_emb,
                                            values=seq_emb,
                                            num_units=self.params['block_shape'][0]*5,
        ...
        # target attention pooling
            ...
            flat = self.attention_sum_pooling(target_embedding, seq_emb, dims=self.params['block_shape'][-1]*5, flag=flag)
        else:
            flat = tf.reduce_mean(seq_emb, axis=1)
        return flat
    ...
    def train(self, features, sess, data_type):
        ...
        loss, train_op = sess.run([self.loss, self.train_op],
                                  ...
                                  self.hiscate1ids:features[6], self.hiscate2ids:features[7], self.hisyearids:features[8],
                                  self.hisstarsids:features[9],self.label:features[10],self.uid:features[11],self.gender_id:features[12],
