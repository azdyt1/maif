...
for user in tqdm(user_interaction.keys()):
    ...
    user_interaction[user] = sorted(interaction_list, key=lambda x:x['dt'])
...
file_item = open('movie_item_map.txt', 'w')
...
top_50_item = [item_map[i[0]] for i in sort_item_list[:50]]
top_1000_item = [item_map[i[0]] for i in sort_item_list[:200]]
...
# user_info = np.load('movie1m_user_info_all.npz', allow_pickle=True)
...
file_train = open('movie_traindata_negall_side_200_2.txt','w')
file_valid = open('movie_validdata_08_side_200_2.txt','w')
file_test = open('movie_testdata_08_side_200_2.txt','w')
...
for user in tqdm(train_user):
    ...
    for index in range(len(interaction_list)):
        ...
        train_sample_cnt += 1
        if index<=25:
            file_train.write(str(query_id)+'\t'+user+'\t'+str(item_id)+'\t'+'-'.join(his_item_list[:index])+'\t1\t'+dt+'\n')
        else:
            file_train.write(str(query_id) + '\t' + user + '\t' + str(item_id) + '\t' + '-'.join(
                his_item_list[index-25:index]) + '\t1\t' + dt + '\n')
        ...
        for j in range(1):
            ...
            while neg in his_item_map.keys():
                neg = random.choice(neg_all)
            if index <= 25:
                file_train.write(str(query_id) + '\t' + user + '\t' + str(neg) + '\t' + '-'.join(his_item_list[:index]) + '\t0\t' + dt + '\n')
            else:
                file_train.write(str(query_id) + '\t' + user + '\t' + str(neg) + '\t' + '-'.join(
                    his_item_list[index-25:index]) + '\t0\t' + dt + '\n')
...
for user in tqdm(valid_user):
    ...
    for index in range(split_index, min(len(interaction_list), split_index+2)):
        item = interaction_list[index]
        item_id = item_map[item['item_id']]
        dt = item['dt']
        # if dt <= '2019-12-01':
        valid_sample_cnt += 1
        if split_index<=25:
            file_valid.write(str(query_id)+'\t'+user+'\t'+str(item_id)+'\t'+'-'.join(his_item_list[:split_index])+'\t1\t'+dt+'\n')
        else:
            file_valid.write(str(query_id) + '\t' + user + '\t' + str(item_id) + '\t' + '-'.join(
                his_item_list[split_index-25:split_index]) + '\t1\t' + dt + '\n')

    for j in range(50):
        valid_sample_cnt += 1
        neg = random.choice(top_1000_item)
        while neg in his_item_map.keys():
            neg = random.choice(neg_all)
        # neg = top_50_item[j]
        if split_index <= 25:
            file_valid.write(str(query_id) + '\t' + user + '\t' + str(neg) + '\t' + '-'.join(his_item_list[:split_index]) + '\t0\t' + dt + '\n')
        else:
            file_valid.write(str(query_id) + '\t' + user + '\t' + str(neg) + '\t' + '-'.join(
                his_item_list[split_index-25:split_index]) + '\t0\t' + dt + '\n')
...
for user in tqdm(test_user):
    ...
    for index in range(split_index, min(len(interaction_list), split_index+2)):
        ...
        test_sample_cnt += 1
        if split_index <= 25:
            file_test.write(str(query_id)+'\t'+user+'\t'+str(item_id)+'\t'+'-'.join(his_item_list[:split_index])+'\t1\t'+dt+'\n')
        else:
            file_test.write(str(query_id) + '\t' + user + '\t' + str(item_id) + '\t' + '-'.join(
                his_item_list[split_index-25:split_index]) + '\t1\t' + dt + '\n')
    ...
    for j in range(50):
        ...
        # neg = top_50_item[j]
        if split_index <= 25:
            file_test.write(str(query_id) + '\t' +user + '\t' + str(neg) + '\t' + '-'.join(his_item_list[:split_index]) + '\t0\t' + dt + '\n')
        else:
            file_test.write(str(query_id) + '\t' + user + '\t' + str(neg) + '\t' + '-'.join(
                his_item_list[split_index-25:split_index]) + '\t0\t' + dt + '\n')
